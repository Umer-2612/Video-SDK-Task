import { INotificationCreate, NotificationType } from "../interfaces/notification.interface";
...
export class NotificationValidationService {
  ...
  public async validateNotification(data: INotificationCreate): Promise<ValidationResult> {
    const errors: string[] = [];
  ...
  public async validateChannel(type: NotificationType, userId: string): Promise<boolean> {
    try {
